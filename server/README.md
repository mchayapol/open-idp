...
1. Sign up
```
curl -X POST -d '{"username":"mchayapol","password":"mchayapol"}' localhost:8000/auth/sign-up
```
2. Sign In
```
curl -X POST -d '{"username":"mchayapol","password":"mchayapol"}' localhost:8000/auth/sign-in
```
and obtain token
```
...
5. Get all bookmarks
```
curl -v -X GET -H "Content-Type: application/json" -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MjE1NDQ2OTguNTc3OTc1LCJ1c2VyIjp7IklEIjoiNjY5YjVlMTQ5MGQ1ZTYxZTAxZmQxNmY1IiwiVXNlcm5hbWUiOiJtY2hheWFwb2wiLCJQYXNzd29yZCI6ImQwNWY0OTg5YjdkYzc1MTdhOWE2MTVkNDQ0ZmZjOGNmNDZhOTU5NTgifX0.-VQRoMqVb-KFJdyQKDgXhpxUh42fpCEE5SjkLKndzV0" localhost:8000/api/bookmarks
```
